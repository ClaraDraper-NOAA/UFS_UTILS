...
! 4.16.20 ToDO: This is a symmetric matrix: can revise later to doing only half of the computations
        ...
        ! incorporate ims corr/cov
        if (assim_IMS) then
            B_cov_mat(num_Obs , 1:num_Obs-1) = 0. 
            B_cov_mat(1:num_Obs-1, num_Obs) = 0. 
        endif
        if (print_debug) then
        ...
        ! if (SNOforc_jndx < snforc_tol) then 
        !     B_cov_mat = Bcov_scale_factor * B_cov_mat 
        !     b_cov_vect = Bcov_scale_factor * b_cov_vect
        ! endif  
        ! if (print_debug) then
        !     print*, "Backround corr at obs pts"
        !     print*, B_cov_mat
        ...
        ! Where(W_wght_vect < 0) W_wght_vect = 0.
        ! if (print_debug) then
        !     print*, "Innov cov"
        !     print*, Innov_cov_mat
        !     print*, "inverse of Innov cov"
    ...
    SUBROUTINE Snow_DA_OI(back_at_Obs, obs_Array, num_Obs,  &
        ...
        !if (IEEE_IS_NAN(back_at_Grid)) anl_at_Grid = IEEE_VALUE(anl_at_Grid, IEEE_QUIET_NAN)
