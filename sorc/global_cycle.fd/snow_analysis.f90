...
!  IMS_INDEXES_PATH="/scratch2/BMC/gsienkf/Tseganeh.Gichamo/SnowObs/IMS_INDEXES/",
        ...
        REAL                :: SNOFCS(LENSFC), VETFCS(LENSFC)
        ...
        Integer, Allocatable   :: Loc_backSt_atObs(:), index_back_atIMS(:)
...
! 2b. Read remotely sensed snow cover, and convert to  snow depth or SWE. 
       ...
       if (assim_IMS) then
            ...
            If (IDIM == 96) then   
                num_sub = 627               
                max_distance = 240.     !Km distance from gridcell to search for corresponding background state
...
!=============================================================================================
        ...
        ims_assm_hour = 18  
        max_distance = 250. ! radius of observation search
        ...
        Do jndx = mp_start, mp_end     
                ...
                ! if (assim_IMS .AND. (IH == ims_assm_hour)) then
                !         Call nearest_IMS_Locations(LENSFC, max_num_loc, max_distance, &
		...
		if (assim_IMS) then
                ...
                !---11.12.20 commented out to see if the multi-ims solves issue
...
999 CONTINUE
 ...
 SUBROUTINE READ_Forecast_Data_atPath(forc_inp_path, MYRANK, LENSFC, SWEFCS, SNDFCS) !, VETFCS) !VEGFCS, !SRFLAG)
